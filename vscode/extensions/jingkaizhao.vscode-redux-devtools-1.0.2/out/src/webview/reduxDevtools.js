...
function createReduxDevtoolsPanel(context) {
    const settings = vscode.workspace.getConfiguration('reduxdev');
    const socketOptions = {
        hostname: settings.hostname || 'localhost',
        port: settings.port || 8000
    };
    const panel = vscode.window.createWebviewPanel('vscode-redux-devtools', 'Remote Devtools', vscode.ViewColumn.Two, { enableScripts: true });
    const reduxDevtoolsCorePath = vscode.Uri.file(path.join(context.extensionPath, 'externals', 'redux-devtools-core.min.js'));
    const reactPath = vscode.Uri.file(path.join(context.extensionPath, 'externals', 'react.production.min.js'));
    const reactDomPath = vscode.Uri.file(path.join(context.extensionPath, 'externals', 'react-dom.production.min.js'));
...
exports.createReduxDevtoolsPanel = createReduxDevtoolsPanel;
function getDevtoolContent(externals, socketOptions) {
    return `<!DOCTYPE html>
<html>
  <head>
...
exports.getDevtoolContent = getDevtoolContent;
